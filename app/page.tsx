...
type SpecialDetail = {title:string;kicker:string;body:string;note:string;items?:string[];rules?:string[]};
const happyHourFood = ["Chips & guacamole", "Chips & queso", "Chips & salsa", "Empanadas", "Cheese quesadilla", "Esquites", "Ceviche tostada", "Mushroom taco"];
const ayceTacos = ["Barbacoa", "Baja fish", "Blackened fish", "Carne asada", "Carnitas", "Chingada", "Chorizo", "Fried chicken", "Halloumi"];
const ayceRules = ["Choose three tacos for your first round", "Order one taco at a time after the first round", "One-hour dining limit", "No sharing", "Dine-in only"];
const specialDetails: Record<string, SpecialDetail> = {
  happy: {title:"Happy Hour Small Bites", kicker:"Every day · 4–7 PM", body:"Eight proper La Chingada snacks—made for the table, not hidden behind a generic link.", items:happyHourFood, note:"The Happy Hour food menu is available all day Wednesday. Dine-in only · Selection may change."},
  ayce: {title:"All You Can Eat Tacos", kicker:"Every Thursday · $29.95 per person", body:"Nine taco choices, fresh tortillas and one very serious hour of eating.", items:ayceTacos, rules:ayceRules, note:"Availability and restaurant conditions apply. Ask your server about dietary needs before ordering."},
  ...
  Wednesday: {title:"Hump Day Small Bites", kicker:"Wednesday · All day", body:"The complete Happy Hour food menu is available all day Wednesday.", items:happyHourFood, note:"Dine-in only · Selection may change."},
  Thursday: {title:"AYCE Tacos", kicker:"Thursday · $29.95 per person", body:"Nine taco choices, served one round at a time.", items:ayceTacos, rules:ayceRules, note:"Availability and restaurant conditions apply."},
...
export default function Home() {
  ...
  return <main>
    ...
    {openSpecial && specialDetails[openSpecial] && (()=>{const detail=specialDetails[openSpecial];return <div className="modal special-modal" onClick={()=>setOpenSpecial(null)}><article onClick={e=>e.stopPropagation()}><button className="close" aria-label="Close special details" onClick={()=>setOpenSpecial(null)}>×</button><p className="eyebrow">{detail.kicker}</p><h2>{detail.title}</h2><p>{detail.body}</p>{detail.items&&<div className="special-items"><strong>{openSpecial==="ayce"||openSpecial==="Thursday"?"CHOOSE YOUR TACOS":"THE FOOD MENU"}</strong><ul>{detail.items.map(item=><li key={item}>{item}</li>)}</ul></div>}{detail.rules&&<div className="special-rules"><strong>HOW IT WORKS</strong><ol>{detail.rules.map(rule=><li key={rule}>{rule}</li>)}</ol></div>}<div className="torn-note">{detail.note}</div><div className="modal-actions"><a className="button red" href="#menu" onClick={()=>setOpenSpecial(null)}>Browse the full menu</a><a className="button paper" href="mailto:reservations@lachingada.ca">Reserve a table</a></div></article></div>})()}
